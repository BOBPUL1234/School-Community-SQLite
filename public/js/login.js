const BASE_URL = location.origin.includes("onrender") 
...
function registerStudent() {
    ...
    fetch(BASE_URL + "/auth/signup/student", {
        ...
        body: JSON.stringify({ id, name, password })  // ✅ 필드명 통일
    })
    .then(res => res.json())
    .then(data => {
        alert(data.message);
        if (data.success) window.location.href = data.redirect || "/main.html";
    })
...
function registerTeacher() {
    ...
    fetch(BASE_URL + "/auth/signup/teacher", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name, password, key: securityKey })
    })
    .then(res => res.json())
    .then(data => {
        alert(data.message);
